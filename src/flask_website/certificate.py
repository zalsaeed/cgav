...
class Certificate:
    ...
    def certificate_info(self, intro_x: float, intro_y: float, intro_w: float, intro_h: float,intro_color: str,
        ...
        # intro
        self.ax.text(intro_x, intro_y,get_display(reshape(self.certificate_intro)), fontsize=DEFAULT_FONT[1], color=intro_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONT[0]))
        ...
        self.ax.text(name_x, name_y,get_display(reshape(self.recipient_name)) ,fontsize=DEFAULT_FONT[1], color=name_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONT[0]))
        ...
        # recipient_title
        self.ax.text(title_x,title_y,get_display(reshape(self.recipient_title)) ,fontsize=DEFAULT_FONT[1], color=title_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONT[0]))
        ...
        for line in body_lines:
            self.ax.text(body_x, y_position, get_display(reshape(line)), fontsize=DEFAULT_FONT[1], color= body_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONT[0]))
    ...
    def signature_filed(self, x: float, y: float, h: float, w: float, name: str, color: str, position: str, signature: str):
        ...
        self.ax.text(x,  y-0.04,get_display(reshape(name)), fontsize=FONT[1], color=color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=FONT[0]))
        self.ax.text(x,  y,get_display(reshape(position)) , fontsize=FONT[1], color=color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=FONT[0]))
        # self.pdf.set_xy(x, y)
    ...
    def greeting(self, greeting_txt: str, x:float , y: float, color:str):
        ...
        self.ax.text(x, y,get_display(reshape(greeting_txt)), fontsize=6, color=color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=font_path))
    ...
    def contact_info(self, contact_info: Dict[int, List[str]], x: float, y: float, color:str):
        ...
        for key, value in contact_info.items():
            ...
            imagebox = OffsetImage(img, zoom=zoom)
            ab_image = AnnotationBbox(imagebox, xy=(x, y-add),xybox=(x-0.07, y-add),boxcoords='axes fraction', frameon=False)
            self.ax.add_artist(ab_image)
    ...
    def generate_certificate(self, output_dir: str,get_certificate_customizations):
        ...
        if customization:
            ...
            self.certificate_info(intro_x, intro_y, intro_w, intro_h,intro_color,name_x, name_y, name_w, name_h,name_color,
            ...
            self.signature_filed(signature_1_x, signature_1_y, signature_1_h, signature_1_w, self.dean_name,signature_1_color, self.dean_position,
            ...
            if (self.path_to_csu_head_signature is not None):
                self.signature_filed(signature_2_x, signature_2_y,signature_2_h, signature_2_w, self.csu_director_name, signature_2_color,self.csu_position,
            ...
            self.fig.savefig(output_pdf_path, format='pdf', dpi=300, bbox_inches='tight', pad_inches=0, metadata={'Title': 'Arabic PDF'})
...
class BilingualCertificate:
    ...
    def certificate_info(self, 
        ...
        # intro
        self.ax.text(intro_x, intro_y,get_display(reshape(self.certificate_intro)), fontsize=DEFAULT_FONTB[1], color=intro_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONTB[0]))

        # self.pdf.set_xy(intro_x, intro_y)
        # self.pdf.cell(intro_w, intro_h, get_display(reshape(self.certificate_intro)), border=0, align="C", fill=False)
        
        # intro_en
        self.ax.text(intro_x_en, intro_y_en,get_display(reshape(self.intro_en)), fontsize=DEFAULT_FONTB[1], color=intro_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONTB[0]))
        ...
        # recipient_name
        self.ax.text(name_x, name_y,get_display(reshape(self.recipient_name)) ,fontsize=DEFAULT_FONTB[1], color=name_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONTB[0]))
        ...
        # recipient_name_en
        self.ax.text(name_x_en, name_y_en,get_display(reshape(self.recipient_name_en)) ,fontsize=DEFAULT_FONTB[1], color=name_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONTB[0]))
        ...
        # recipient_title
        self.ax.text(title_x,title_y,get_display(reshape(self.recipient_title)) ,fontsize=DEFAULT_FONTB[1], color=title_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONTB[0]))
        # recipient_title_width = self.pdf.get_string_width(self.recipient_title)
        # left_margin_title = (title_w - recipient_title_width) / 2
        # self.pdf.set_xy(title_x + left_margin_title, title_y)
        ...
        # recipient_title_en
        self.ax.text(title_x_en,title_y_en,get_display(reshape(self.recipient_title_en)) ,fontsize=DEFAULT_FONTB[1], color=title_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONTB[0]))
        ...
        y_position = body_y
        for line in body_lines:
            self.ax.text(body_x, y_position, get_display(reshape(line)), fontsize=DEFAULT_FONTB[1], color= body_color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=DEFAULT_FONTB[0]))
            y_position -= 0.06 # Move to the next line
        # body_lines = util.break_string_into_chunks(self.certificate_body, 58)
        # for idx, line in enumerate(body_lines):
        #     self.pdf.set_xy(body_x, body_y + (body_h * idx))
    ...
    def greeting(self,greeting_x_en, greeting_x, greeting_txt: str, y: float,color:str):
        font_path="fonts/Amiri/Amiri-Regular.ttf"
        self.ax.text(greeting_x, y,get_display(reshape(self.greeting_txt_en)), fontsize=4, color=color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=font_path))
        self.ax.text(greeting_x_en, y,get_display(reshape(greeting_txt)), fontsize=4, color=color, ha='center', va='center', transform=self.ax.transAxes,fontproperties=FontProperties(fname=font_path))
        # self.pdf.set_font("Amiri", size=12)
        # self.pdf.set_font("Amiri", size=12)
        # self.pdf.set_text_color(5, 168, 172)  # text color
